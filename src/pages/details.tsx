...
const Details = () => {
  ...
  useEffect(() => {
    // If a product was passed via location state (e.g., from a category click), use it
    ...
    // Otherwise, try to resolve by id param
  ...
  const discountPercentage = product.originalPrice > 0 ? Math.round(((product.originalPrice - product.price) / product.originalPrice) * 100) : 0;
